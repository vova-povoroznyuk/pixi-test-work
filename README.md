...
- Політики вибору дока/черги (`DockAssignmentPolicy.ts`)
- Алгоритм черги (зараз FIFO, можна зробити слоти, пріоритети, ліміти)
- Маршрути доків (`DockViewFactory.ts`)
- Логіку loading (замість таймера, наприклад, проміс/ресурсна модель)
